...
public class CreateTable {
	...
	//private static String tableName = "tableKudu";
    ...
    public static void kuduSave() {
		KuduClient client = new KuduClient.KuduClientBuilder(KUDU_MASTER).build();
		try {
			KuduTable table = client.openTable("tableKudu");
			KuduSession session = client.newSession();
			System.out.println("-------start--------" + System.currentTimeMillis());
			for (short i = 1; i < 100; i++) {
				Insert insert = table.newInsert();
				PartialRow row = insert.getRow();
				row.addShort("CompanyId", i);
    ...
    public static void kuduCreateTable2() {
       ...
       //options.addHashPartitions(parcols, 3);
       try {
           client.createTable("WA_SOURCE_FJ_1001", schema, options);
       } catch (KuduException e) {
           e.printStackTrace();
       }
       try {
